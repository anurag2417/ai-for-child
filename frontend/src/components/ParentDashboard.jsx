...
function StatCard({ icon: Icon, label, value, color, testId }) {
  ...
  return (
    <div
      data-testid={testId}
      className="bg-slate-900 rounded-3xl shadow-[0_12px_35px_rgb(0,0,0,0.45)] border-2 border-slate-700 p-6 hover:shadow-[0_8px_30px_rgb(0,0,0,0.08)] transition-all duration-300"
    >
      <div className="flex items-center gap-4">
        ...
        <div>
          ...
          <p className="font-['Nunito'] text-3xl font-extrabold text-slate-100">{value}</p>
        </div>
      </div>
    </div>
  );
...
export default function ParentDashboard() {
  ...
  return (
    <div data-testid="parent-dashboard" className="min-h-screen bg-slate-950">
      {/* Top Bar */}
      <header
        data-testid="parent-header"
        className="bg-slate-900/80 backdrop-blur-xl border-b border-slate-700 px-6 py-4 flex items-center gap-4 sticky top-0 z-10"
      >
        ...
        <h1 className="font-['Nunito'] text-2xl font-extrabold text-slate-100">Parent Dashboard</h1>
        ...
        {user && (
          <div className="flex items-center gap-3">
            ...
            <button
              ...
              onClick={logout}
              className="p-2 rounded-xl hover:bg-slate-800 transition-colors text-slate-400 hover:text-rose-500"
              title="Logout"
            >
      ...
      <div className="max-w-6xl mx-auto px-4 md:px-8 py-8">
        ...
        {activeTab === "overview" && (
          <div data-testid="overview-content">
            ...
            {/* Recent Alerts */}
            <div className="bg-slate-900 rounded-3xl shadow-[0_12px_35px_rgb(0,0,0,0.45)] border-2 border-slate-700 p-6">
              <h3 className="font-['Nunito'] text-xl font-bold text-slate-100 mb-4 flex items-center gap-2">
        ...
        {activeTab === "alerts" && (
          <div data-testid="alerts-content" className="space-y-4">
            <h3 className="font-['Nunito'] text-2xl font-bold text-slate-100 mb-2">All Alerts</h3>
            {alerts.length === 0 ? (
              <div className="bg-slate-900 rounded-3xl p-12 text-center border-2 border-slate-700">
        ...
        {activeTab === "conversations" && (
          <div data-testid="conversations-content" className="space-y-3">
            <h3 className="font-['Nunito'] text-2xl font-bold text-slate-100 mb-2">Conversation Logs</h3>
            {conversations.length === 0 ? (
              ...
              conversations.map((conv) => (
                <Link
                  ...
                  className="block bg-slate-900 rounded-2xl p-5 border-2 border-slate-700 hover:shadow-[0_8px_30px_rgb(0,0,0,0.06)] transition-all duration-200 group"
                >
                  <div className="flex items-center justify-between">
                    <div className="flex items-center gap-3">
                      <MessageCircle className="w-6 h-6 text-sky-400" strokeWidth={2.5} />
                      <div>
                        <p className="font-bold text-slate-100 text-lg">{conv.title}</p>
                    ...
                    <div className="flex items-center gap-3">
                      ...
                      <ChevronRight className="w-5 h-5 text-slate-400 group-hover:text-sky-400 transition-colors" />
                    </div>
                  </div>
                </Link>
              ))
        ...
        {activeTab === "browsing" && (
          <div data-testid="browsing-content" className="space-y-6">
            <div className="flex items-center justify-between">
              <h3 className="font-['Nunito'] text-2xl font-bold text-slate-100">Browsing Activity</h3>
              <button
            ...
            {browsingStats && (
              <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
                <div className="bg-slate-900 rounded-2xl p-4 border-2 border-slate-700 text-center">
                  <Search className="w-6 h-6 text-sky-400 mx-auto mb-2" strokeWidth={2.5} />
                  <p className="font-['Nunito'] text-2xl font-extrabold text-slate-100">{browsingStats.search_count}</p>
                ...
                <div className="bg-slate-900 rounded-2xl p-4 border-2 border-slate-700 text-center">
                  ...
                  <p className="font-['Nunito'] text-2xl font-extrabold text-slate-100">{browsingStats.visit_count}</p>
                ...
                <div className="bg-slate-900 rounded-2xl p-4 border-2 border-slate-700 text-center">
                  ...
                  <p className="font-['Nunito'] text-2xl font-extrabold text-slate-100">{browsingStats.incognito_count}</p>
                ...
                <div className="bg-slate-900 rounded-2xl p-4 border-2 border-slate-700 text-center">
                  ...
                  <p className="font-['Nunito'] text-2xl font-extrabold text-slate-100">{browsingStats.flagged_searches}</p>
            ...
            {browsingAnalysis && (
              <div data-testid="browsing-analysis" className={`rounded-3xl p-6 border-2 ${
                ...
                <p className="text-base font-medium text-slate-200 mb-2">{browsingAnalysis.analysis}</p>
                {browsingAnalysis.positive && (
                  <p className="text-sm font-medium text-emerald-700 bg-emerald-100 rounded-xl p-3 mt-2">
                    {browsingAnalysis.positive}
                  </p>
                )}
                <div className="flex gap-4 mt-3 text-sm font-semibold text-slate-400">
                  ...
                  <div className="mt-4">
                    ...
                    <div className="space-y-2">
                      {browsingAnalysis.flagged_searches.map((s, i) => (
                        <div key={i} className="bg-slate-900/80 rounded-xl p-3 border border-slate-700 text-sm">
                          <span className="font-bold text-slate-200">"{s.query}"</span>
                          {s.tab_type === "incognito" && (
                            <span className="ml-2 text-xs font-bold px-2 py-0.5 rounded-full bg-violet-100 text-violet-200">Incognito</span>
                          )}
            ...
            {/* Recent Searches */}
            <div className="bg-slate-900 rounded-3xl shadow-[0_12px_35px_rgb(0,0,0,0.45)] border-2 border-slate-700 p-6">
              <h4 className="font-['Nunito'] text-lg font-bold text-slate-100 mb-4 flex items-center gap-2">
              ...
              {browsingSearches.length === 0 ? (
                <div className="text-center py-8">
                  ...
                  <p className="text-slate-400 text-sm font-medium mt-1">Install the BuddyBot Chrome extension to start tracking.</p>
                </div>
              ) : (
                <div className="space-y-2">
                  {browsingSearches.map((s, i) => (
                    ...
                    >
                      ...
                      <div className="flex items-center gap-2 flex-shrink-0">
                        {s.tab_type === "incognito" && (
                          <span className="text-xs font-bold px-2 py-0.5 rounded-full bg-violet-100 text-violet-200">Incognito</span>
                        )}
                        {s.profanity_flagged && (
                  ...
                  {browsingVisits.slice(0, 15).map((v, i) => (
                    <div key={v.id || i} data-testid={`visit-packet-${i}`} className="flex items-center gap-3 p-3 rounded-xl bg-slate-900 border border-slate-700">
                      ...
                      {v.tab_type === "incognito" && (
                        <span className="text-xs font-bold px-2 py-0.5 rounded-full bg-violet-100 text-violet-200 flex-shrink-0">Incognito</span>
                      )}
                    </div>
                  ))}
                </div>
  ...
  return (
    <div
      ...
      className={`rounded-2xl p-5 border-2 flex flex-col md:flex-row md:items-center gap-4 ${
        alert.resolved ? "bg-slate-900 border-slate-700" : "bg-rose-50 border-rose-200"
      }`}
    >
      <div className="flex-1 min-w-0">
        <div className="flex items-center gap-3 mb-2 flex-wrap">
          ...
          {isExtension && (
            <span className="text-xs font-bold px-2 py-0.5 rounded-full bg-violet-100 text-violet-200">Browser</span>
          )}
        ...
        <p className="text-base font-medium text-slate-200 truncate">{alert.details}</p>
      ...
      {!alert.resolved && (
        <button
          ...
          className="bg-emerald-400 hover:bg-emerald-900/300 text-white rounded-full px-5 py-2.5 text-sm font-bold shadow-[0_3px_0_0_rgba(16,185,129,0.3)] hover:translate-y-[1px] hover:shadow-[0_1px_0_0_rgba(16,185,129,0.3)] active:translate-y-[3px] active:shadow-none transition-all duration-150 whitespace-nowrap"
        >
          Mark Resolved
        </button>
      )}
